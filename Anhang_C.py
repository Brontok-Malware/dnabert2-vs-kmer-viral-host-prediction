...
train_loader = DataLoader(train_dataset, batch_size=64, shuf-fle=True)
...
for epoch in range(epochs):
    ...
    if (epoch + 1) % 5 == 0:
        print(f"Epoche [{epoch+1}/{epochs}], Loss: {to-tal_loss/len(train_loader):.4f}")
...
# Matrix für Random Forest
cm_rf = confusion_matrix(y_test_encoded, rf_preds)
disp_rf = ConfusionMatrixDisplay(confusion_matrix=cm_rf, display_labels=["Non-Human", "Human"])
disp_rf.plot(cmap="Blues", values_format="d")
plt.title("Konfusionsmatrix: Random Forest (k=4)")
plt.savefig("cm_rf.png", dpi=300, bbox_inches='tight')
plt.close()

# Matrix für MLP
cm_mlp = confusion_matrix(all_targets, all_preds)
disp_mlp = ConfusionMatrixDisplay(confusion_matrix=cm_mlp, display_labels=["Non-Human", "Human"])
disp_mlp.plot(cmap="Blues", values_format="d")
plt.title("Konfusionsmatrix: Multi-Layer Perceptron (k=4)")
plt.savefig("cm_mlp.png", dpi=300, bbox_inches='tight')
plt.close()

print("Matrizen für Baselines gespeichert!")